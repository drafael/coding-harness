...
async function hasJavaProjectMarker(directory: string): Promise<boolean> {
  const checks = JAVA_PROJECT_MARKERS.map((marker) =>
...
export async function findJavaProjectRoot(
  cwd: string,
  ...
  return (await containsJavaSource(startingDirectory))
    ? startingDirectory
    : undefined;
}
...
export default function autoJavaCoder(pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    if (!detectionComplete) {
      javaProjectRoot = await findJavaProjectRoot(
    ...
    const discoveredSkill = event.systemPromptOptions.skills?.find(
      (skill) => skill.name === "java-coder",
    );
    const skillPath =
      discoveredSkill?.filePath ??
      join(homedir(), ".agents", "skills", "java-coder", "SKILL.md");
    ...
    try {
      ...
      if (!missingSkillReported) {
        ...
        ctx.ui.notify(
          `Could not auto-activate java-coder: ${message}`,
          "error",
        );
