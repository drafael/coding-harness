...
- Resolve `scripts/export_png.py` from the installed skill root and run it with the selected Python interpreter and `--check`.
- The exporter discovers Python Playwright environments and Playwright-managed or system Chromium-family browsers. Do not duplicate that logic with an active-interpreter import check or browser-cache heuristic.
- If the command passes, report the exact Python interpreter, browser path, and browser version from its output.
- If it fails, mark `warn` and preserve its interpreter-specific install commands or `DIAGRAM_DESIGN_CHROMIUM` override guidance in the fix.
- Never auto-install dependencies or download a browser.
...
[WARN] No Python Playwright installation discovered
[PASS] scripts/verify-drawio-import.py present
...

Next actions
- Install PNG export dependencies: /usr/bin/python3 -m pip install playwright
- Install managed Chromium if no system browser is available: /usr/bin/python3 -m playwright install chromium
