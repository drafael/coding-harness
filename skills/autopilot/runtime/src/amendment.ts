...
function pendingCommitIntent(
  records: readonly JournalRecord[],
  itemId: string,
): { readonly idempotencyKey: string; readonly expectedTree: string; readonly attemptId: string } | undefined {
  const confirmed = new Set(records.flatMap(({ event }) =>
    event.type === "EFFECT_CONFIRMED" && event.itemId === itemId && event.effect === "git.commit" ? [event.idempotencyKey] : []
  ));
  return records.flatMap(({ event }) =>
    event.type === "EFFECT_INTENDED" && event.itemId === itemId && event.effect === "git.commit"
...
export async function loadAmendmentContext(
  stateRoot: string,
  charter: RunCharter,
  currentRecords: readonly JournalRecord[] = [],
): Promise<AmendmentContext | undefined> {
  const amendment = charter.amends;
  if (amendment === undefined) {
    return undefined;
  }
  const predecessorDirectory = runDirectory(stateRoot, amendment.runId);
  let predecessorCharter: RunCharter;
  try {
    predecessorCharter = parseSealedCharter(JSON.parse(await readFile(join(predecessorDirectory, "charter.json"), "utf8")) as unknown);
  } catch (error) {
  ...
  const adoptedPath = journal.records.flatMap(({ event }) =>
    event.type === "WORKTREE_ADOPTED" && event.itemId === amendment.itemId ? [event.worktreePath] : []
  ).at(-1);
  ...
  let reconciledCommit: AmendmentContext["reconciledCommit"];
