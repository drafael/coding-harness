...
export async function resolveCommit(repositoryRoot: string, ref: string): Promise<string> {
  return await git(repositoryRoot, ["rev-parse", "--verify", `${ref}^{commit}`]);
}

export async function currentBranch(repositoryRoot: string): Promise<string> {
...
export async function inspectCommit(
  ...
  commit: string,
): Promise<{ readonly parents: readonly string[]; readonly treeIdentity: string; readonly message: string }> {
...
export async function inspectRepository(repositoryRoot: string): Promise<{ readonly headCommit: string; readonly clean: boolean }> {
...
export async function validateBaseCommit(charter: RunCharter): Promise<void> {
...
export async function branchExists(repositoryRoot: string, branchName: string): Promise<boolean> {
...
export async function validateBranchName(repositoryRoot: string, branchName: string): Promise<void> {
...
export async function assertRegisteredWorktree(repositoryRoot: string, worktreePath: string): Promise<void> {
...
export async function ensureWorktree(
  charter: RunCharter,
  item: WorkItem,
  baseCommit = charter.repository.baseCommit,
...
async function changedPaths(worktreePath: string): Promise<readonly string[]> {
...
function lexicalWithin(path: string, root: string): boolean {
...
export async function assertWritablePaths(worktreePath: string, changed: readonly string[], writableRoots: readonly string[]): Promise<void> {
...
export async function remoteBranchCommit(repositoryRoot: string, remote: string, branchName: string): Promise<string | undefined> {
  const result = await runProcess({
    executable: "git",
...
export async function pushAmendmentBranch(
  worktreePath: string,
  remote: string,
  branchName: string,
...
export async function inspectPreCommitHook(worktreePath: string): Promise<PreCommitHookSnapshot> {
...
export async function runPreCommitHook(
  worktreePath: string,
  expectedHook: PreCommitHookSnapshot,
...
export async function commitAcceptedWork(
  worktreePath: string,
  charter: RunCharter,
  item: WorkItem,
  attemptId: string,
