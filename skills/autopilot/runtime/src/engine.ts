...
import type { GrantFamily, RunCharter, WorkItem } from "./charter.js";
...
import { reduce, type RunProjection } from "./reducer.js";
import {
  assertWritablePaths,
  branchExists,
  commitAcceptedWork,
...
export class AutopilotEngine {
  ...
  async #deliverItem(item: WorkItem, worktreePath: string, expectedCommit: string): Promise<boolean> {
    ...
    const checksStatus = checks.length === 0
      ? "UNVERIFIED" as const
      : checks.every(({ subjectCommit, status }) => subjectCommit === expectedCommit && status === "passed") ? "PASSED" as const : "FAILED" as const;
  ...
  async #runItem(item: WorkItem): Promise<void> {
    ...
    if (!finalObservation.clean) {
      ...
      acceptedCommit = await commitAcceptedWork(
        worktreePath,
        this.#charter,
        item,
        attemptId,
        finalObservation.treeIdentity,
        finalObservation.headCommit,
      );
      await this.#record({
