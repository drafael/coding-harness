...
import { acquireBranchOwnershipLock, acquireRunLock, type RunLock } from "./lock.js";
...
import { loadAvailableRuns, loadStoredRun, type StoredRun } from "./run-discovery.js";
import { runDirectory } from "./state-path.js";

type LoadedRun = StoredRun;
...
export interface WrapUpDiscovery {
  ...
  readonly excluded: readonly { readonly runId: string; readonly reason: string }[];
  readonly corrupt: readonly { readonly name: string; readonly reason: string }[];
}
...
interface ItemCleanup {
  ...
  readonly legacyLeaseWorktree: boolean;
}
...
function changeRequestState(records: readonly JournalRecord[], itemId: string): string | undefined {
  return records.flatMap(({ event }) =>
    event.type === "EFFECT_CONFIRMED" && event.itemId === itemId
...
function changeRequestRef(provider: "github" | "gitlab", url: string): ChangeRequestRef {
...
function runSucceeded(run: LoadedRun): boolean {
  return rebuildProjection(run.charter, run.records).state === "SUCCEEDED";
}

async function cleanWrapUpTrash(stateRoot: string): Promise<void> {
...
export async function discoverWrapUpRuns(stateRoot: string, repositoryRoot: string): Promise<WrapUpDiscovery> {
  ...
  const excluded: { runId: string; reason: string }[] = [];
...
async function loadAmendmentChain(stateRoot: string, target: LoadedRun): Promise<readonly LoadedRun[]> {
  const chain = [target];
...
async function latestLeafCheck(stateRoot: string, runId: string): Promise<void> {
...
async function resolveItemCleanup(stateRoot: string, run: LoadedRun, item: WorkItem): Promise<ItemCleanup> {
  const target = run.charter.deliveryTarget;
  if (target === undefined) {
  ...
  const adoptedPath = run.records.flatMap(({ event }) =>
...
async function validateRetainedWorktree(run: LoadedRun, cleanup: ItemCleanup): Promise<void> {
...
async function preflightItem(run: LoadedRun, cleanup: ItemCleanup): Promise<ItemCleanup> {
  const target = run.charter.deliveryTarget;
  if (target === undefined) {
    throw new AutopilotError("UNSUPPORTED_CAPABILITY", "wrap-up requires a provider delivery target");
  }
...
function effectKey(runId: string, itemId: string | undefined, effect: string): string {
...
async function recordEffect(
  ...
  const intended = records.some(({ event }) =>
    event.type === "EFFECT_INTENDED" && event.effect === effect && event.itemId === itemId && event.idempotencyKey === key
  );
  if (!intended) {
  ...
  if (!records.some(({ event }) =>
    event.type === "EFFECT_CONFIRMED" && event.effect === effect && event.itemId === itemId && event.idempotencyKey === key
  )) {
    await appendEvent(journalPath, {
      eventId: newEventId(),
      timestamp: new Date().toISOString(),
...
async function deleteRemoteBranch(run: LoadedRun, cleanup: ItemCleanup): Promise<string> {
  const target = run.charter.deliveryTarget;
  if (target === undefined) {
    throw new AutopilotError("UNSUPPORTED_CAPABILITY", "wrap-up requires a provider delivery target");
  }
...
async function removeWorktree(run: LoadedRun, cleanup: ItemCleanup): Promise<string> {
  if (!await pathExists(cleanup.worktreePath)) {
    return "absent";
  }
...
async function deleteLocalBranch(run: LoadedRun, cleanup: ItemCleanup): Promise<string> {
  if (!await branchExists(run.charter.repository.root, cleanup.item.branchName)) {
    return "absent";
  }
...
async function handoffObject(run: LoadedRun, chainRunIds: readonly string[], cleanups: readonly ItemCleanup[]): Promise<Record<string, unknown>> {
  ...
  try {
    const report: unknown = JSON.parse(await readFile(join(run.directory, "reports", "status.json"), "utf8"));
...
function handoffMarkdown(handoff: Record<string, unknown>, cleanups: readonly ItemCleanup[]): string {
  const runId = String(handoff.runId);
  const lineage = Array.isArray(handoff.lineage) ? handoff.lineage.map(String).join(" → ") : runId;
  const items = cleanups.map(({ item, acceptedCommit, changeRequest }) =>
    `- **${item.title ?? item.id}** — \`${acceptedCommit}\` — ${changeRequest.url}`
  ).join("\n");
...
async function assertSafeHandoffRoot(repositoryRoot: string): Promise<string> {
...
async function writeIfAbsentOrEqual(path: string, content: string): Promise<void> {
  try {
...
async function prepareHandoff(run: LoadedRun, chainRunIds: readonly string[], cleanups: readonly ItemCleanup[]): Promise<PreparedHandoff> {
...
async function validatePreparedHandoff(handoff: PreparedHandoff): Promise<void> {
...
async function writeHandoff(run: LoadedRun, handoff: PreparedHandoff): Promise<readonly string[]> {
  const directory = await assertSafeHandoffRoot(run.charter.repository.root);
...
async function acquireLocks(stateRoot: string, chainRunIds: readonly string[], branches: readonly string[]): Promise<readonly RunLock[]> {
  const locks: RunLock[] = [];
  try {
...
function wrapUpStarted(run: LoadedRun): Extract<LifecycleEvent, { readonly type: "WRAP_UP_STARTED" }> | undefined {
  const event = run.records.findLast(({ event }) => event.type === "WRAP_UP_STARTED")?.event;
  return event?.type === "WRAP_UP_STARTED" ? event : undefined;
}

async function recordWrapUpStarted(run: LoadedRun, chainRunIds: readonly string[], handoff: boolean): Promise<void> {
...
async function removeCanonicalRunDirectory(stateRoot: string, runId: string): Promise<void> {
  const lock = await acquireRunLock(join(stateRoot, "wrap-up-trash.lock"), "wrap-up trash");
  try {
    const directory = runDirectory(stateRoot, runId);
    if (!await pathExists(directory)) {
...
export async function wrapUpRun(stateRoot: string, runId: string, handoff: boolean): Promise<WrapUpResult> {
  await cleanWrapUpTrash(stateRoot);
  ...
  try {
    ...
    const proposedCleanups = await Promise.all(
      target.charter.work.map(async (item) => await resolveItemCleanup(stateRoot, target, item)),
    );
    const cleanups: ItemCleanup[] = [];
    ...
    for (const cleanup of cleanups) {
      await recordEffect(target, cleanup.item.id, "remote.branch.delete", cleanup.acceptedCommit,
        async () => await deleteRemoteBranch(target, cleanup));
      deletedRemoteBranches.push(cleanup.item.branchName);
    }
    for (const cleanup of cleanups) {
      await recordEffect(target, cleanup.item.id, "git.worktree.remove", cleanup.worktreePath,
        async () => await removeWorktree(target, cleanup));
      removedWorktrees.push(cleanup.worktreePath);
    }
    for (const cleanup of cleanups) {
      await recordEffect(target, cleanup.item.id, "git.branch.delete", cleanup.acceptedCommit,
        async () => await deleteLocalBranch(target, cleanup));
