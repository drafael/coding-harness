...
import { observeReviewFeedback, type ReviewFeedbackSelection } from "./review-feedback.js";
import {
  discoverLifecycleRuns,
  locateStoredRun,
  type LifecycleDiscovery,
  type LifecycleOperation,
} from "./run-discovery.js";
import { resolveStateRoot, runDirectory } from "./state-path.js";
import { discoverWrapUpRuns, wrapUpRun, type WrapUpDiscovery } from "./wrap-up.js";
...
interface CliOptions {
  ...
  readonly recoveryAction?: UnknownRecoveryAction;
  readonly recoveryItem?: string;
  readonly recoveryAttempt?: string;
  readonly recoveryLeaseEpoch?: number;
  readonly recoveryAttestation?: string;
  readonly recoveryTree?: string;
}
...
function formatStatusReport(report: RunReport): string {
...
async function loadRun(runId: string, stateDirectory: string | undefined, repair: boolean): Promise<{
  readonly stateRoot: string;
  readonly directory: string;
  readonly charter: RunCharter;
  readonly journal: Awaited<ReturnType<typeof readJournal>>;
}> {
  const stateRoot = await resolveStateRoot(process.cwd(), stateDirectory);
  let location: Awaited<ReturnType<typeof locateStoredRun>>;
  try {
...
async function runEngine(engine: AutopilotEngine, lock: RunLock, runId: string): Promise<unknown> {
  ...
  let pendingControlCheck: Promise<void> = Promise.resolve();
  const checkControlRequest = (): void => {
...
async function start(charterFile: string, options: CliOptions): Promise<unknown> {
  ...
  try {
    ...
    try {
      ...
      await writeSnapshot(
        join(temporaryDirectory, "snapshot.json"),
        rebuildProjection(charter, initialJournal.records),
...
async function loadReportMetadata(directory: string): Promise<{ readonly assurance: string; readonly unverifiedBoundaries: readonly string[] }> {
  try {
    const value: unknown = JSON.parse(await readFile(join(directory, "reports", "status.json"), "utf8"));
...
async function selectLifecycleRun(
  operation: LifecycleOperation,
  runId: string | undefined,
  options: CliOptions,
): Promise<string | LifecycleDiscovery> {
...
async function status(runId: string | undefined, options: CliOptions): Promise<unknown> {
...
async function wrapUp(runId: string | undefined, options: CliOptions): Promise<unknown> {
  const stateRoot = await resolveStateRoot(process.cwd(), options.stateDir);
  if (runId !== undefined) {
...
function formatLifecycleDiscovery(discovery: LifecycleDiscovery): string {
  const candidates = discovery.candidates.map((candidate) =>
    `${candidate.shortId}\t${candidate.title}\t${candidate.state}\t${candidate.completedItems}/${candidate.totalItems}\t${candidate.updatedAt}\t${candidate.coordinator}`
  );
  const excluded = discovery.excluded.map((candidate) =>
    `- ${candidate.shortId} ${candidate.title}: ${candidate.reason}`
  );
...
function formatReviewFeedbackSelection(selection: ReviewFeedbackSelection): string {
...
function formatWrapUpDiscovery(discovery: WrapUpDiscovery, options: CliOptions): string {
  ...
  const rows = discovery.candidates.map((candidate) =>
    `${candidate.runId}\t${candidate.completedAt}\t${candidate.mode}\t${candidate.provider}\t${candidate.itemCount} item(s)`
  );
...
async function pause(runId: string | undefined, options: CliOptions): Promise<unknown> {
...
async function stop(runId: string | undefined, options: CliOptions): Promise<unknown> {
...
export async function main(arguments_: readonly string[] = process.argv.slice(2)): Promise<number> {
  ...
  const options: CliOptions = {
    ...
    ...(recoveryAction === undefined ? {} : { recoveryAction: recoveryAction as UnknownRecoveryAction }),
  ...
  if (!options.json && isRecord(result) && result.kind === "selection") {
    if (typeof result.operation === "string") {
      output(formatLifecycleDiscovery(result as unknown as LifecycleDiscovery), false);
    } else {
      output(formatWrapUpDiscovery(result as unknown as WrapUpDiscovery, options), false);
    }
  } else if (!options.json && isRecord(result) && result.kind === "review-feedback-selection") {
    output(formatReviewFeedbackSelection(result as unknown as ReviewFeedbackSelection), false);
