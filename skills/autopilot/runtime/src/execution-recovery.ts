...
export async function recoverUnknownExecution(
  runDirectory: string,
  charter: RunCharter,
  ...
  if (request.action === "stop") {
    ...
    let before: Awaited<ReturnType<typeof observeRepository>> | undefined;
    try {
    ...
    const quarantined = await quarantineWorktree(
      charter.repository.root,
      lease.worktreePath,
      charter.runId,
      item.id,
      request.attemptId,
