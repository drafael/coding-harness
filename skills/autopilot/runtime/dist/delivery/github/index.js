...
async function gh(repositoryRoot, arguments_) {
...
async function paginatedGitHubArray(repositoryRoot, endpoint, label) {
    const entries = [];
    let page = 1;
    while (true) {
...
function authorLogin(value, path) {
    if (value === null) {
        return "deleted-user";
    }
...
function reviewThread(value, index) {
...
export class GitHubDeliveryAdapter {
    ...
    async observeChangeRequest(repositoryRoot, ref) {
        const object = expectRecord(parseJson(await gh(repositoryRoot, ["pr", "view", ref.id, "--json", "number,url,state,headRefOid,baseRefName,reviewDecision"]), "gh pr view"), "GitHub pull request");
    ...
    async findChangeRequest(repositoryRoot, runId, itemId) {
        const output = parseJson(await gh(repositoryRoot, ["pr", "list", "--state", "all", "--search", `\"Autopilot-Run: ${runId}\" in:body`, "--json", "number,url,body"]), "gh pr list");
        if (!Array.isArray(output)) {
    ...
    async observeReviewThreads(repositoryRoot, ref) {
        ...
        const issueComments = await paginatedGitHubArray(repositoryRoot, `repos/${owner}/${name}/issues/${ref.id}/comments`, "gh api pull request comments");
        const reviewSummaries = await paginatedGitHubArray(repositoryRoot, `repos/${owner}/${name}/pulls/${ref.id}/reviews`, "gh api pull request reviews");
        const nonResolvable = [...issueComments.map((entry, index) => {
            ...
            }), ...reviewSummaries.flatMap((entry, index) => {
    ...
    async resolveReviewThreads(repositoryRoot, _ref, threadIds) {
    ...
    async observeChecks(repositoryRoot, subjectCommit) {
        ...
        const response = expectRecord(parseJson(await gh(repositoryRoot, ["api", `repos/${ownerAndName}/commits/${subjectCommit}/check-runs`]), "gh api check-runs"), "GitHub check-runs");
