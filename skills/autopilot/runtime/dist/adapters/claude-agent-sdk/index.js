...
import { CLAUDE_AGENT_SDK_CLI_ENVIRONMENT, inspectClaudeAgentSdkInstallation, isClaudeAgentSdkScriptCli, } from "../../src/claude-agent-sdk.js";
...
const IMPLEMENTATION_TOOLS = ["Bash", "Edit", "Glob", "Grep", "Read", "Write"];
const REQUIRED_CAPABILITIES = ["interrupt_receipt_v1", "interrupt_cancel_queued_v1", "msg_lifecycle_v1"];
...
function isInterruptedTerminalReason(value) {
    return value === "aborted_tools" || value === "aborted_streaming";
}
function hasExactUserMessageIdentity(message, userMessageId) {
...
function hasNoUserMessageIdentity(message) {
...
function assistantText(message) {
    ...
    return message.message.content.flatMap((part) => isRecord(part) && part.type === "text" && typeof part.text === "string" ? [part.text] : []).join("");
}
        ...
        for (const byte of chunk) {
            if (byte === 0x0a) {
                ...
                if (this.#lineBytes > this.#maximumLineBytes) {
                    callback(new AutopilotError("EXECUTION_STATE_UNKNOWN", "Claude Agent SDK protocol record exceeded the configured line bound"));
                    return;
                }
            }
        }
...
function subjectMatches(pending, handle) {
...
export class ClaudeAgentSdkAdapter {
    ...
    async launch(request) {
        ...
        let disposal;
        let disposing = false;
        const appendOutput = (text, deduplicateWholeOutput = false) => {
            if (text.length === 0) {
                return;
            }
                                ...
                                await Promise.race([
                                    ...
                                    new Promise((_resolve, reject) => {
                                        closeTimer = setTimeout(() => {
                                            reject(new AutopilotError("EXECUTION_STATE_UNKNOWN", "Claude Agent SDK child streams did not close after direct-child termination"));
        ...
        const validateInit = async (message) => {
            ...
            query = sdk.query({
                ...
                options: {
                    ...
                    hooks: {
                        PreToolUse: [{
                                hooks: [async (input) => {
                                        ...
                                        const allowed = typeof hookInput.tool_name === "string" && IMPLEMENTATION_TOOLS.includes(hookInput.tool_name);
                                        return {
                                            hookSpecificOutput: {
                    ...
                    spawnClaudeCodeProcess: (spawnOptions) => {
                        ...
                        child = spawn(scriptCli ? process.execPath : installation.cliPath, options.args, {
                            cwd: request.worktreePath,
        ...
        const pending = {
            ...
            interrupt: async () => {
                ...
                interruption ??= (async () => {
                    ...
                    const interrupt = interruptRequest.then((receipt) => ({ kind: "receipt", receipt }), (error) => ({ kind: "error", error }));
                    const terminalResult = terminal.promise.then((observation) => ({ kind: "terminal", observation }));
    ...
    async #loadSdk() {
        ...
        let moduleValue;
        try {
            moduleValue = await import(pathToFileURL(installation.modulePath).href);
        }
        catch (error) {
        ...
        return { installation, sdk: moduleValue };
    }
}
