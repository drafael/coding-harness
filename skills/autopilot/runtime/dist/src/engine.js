...
import { assertWritablePaths, branchExists, commitAcceptedWork, ensureWorktree, inspectCommit, inspectPreCommitHook, inspectRepository, installRestackCandidate, observeRepository, prepareRestackCandidate, pushAmendmentBranch, pushBranch, remoteBranchCommit, resolveCommit, resolveWorktreePath, runPreCommitHook, } from "./repository.js";
...
export class AutopilotEngine {
    ...
    #managedBranchExpectations() {
        return this.#charter.work.map((item) => {
            ...
            const confirmedCommit = this.#records.findLast(({ event }) => event.type === "EFFECT_CONFIRMED" && event.itemId === item.id
                && (event.effect === "git.commit" || event.effect === "restack.local-ref"))?.event;
            const predecessorId = item.dependsOn.at(-1);
            const predecessorCommit = predecessorId === undefined
                ? undefined
                : this.#records.findLast(({ event }) => event.type === "EFFECT_CONFIRMED" && event.itemId === predecessorId
                    && (event.effect === "git.commit" || event.effect === "restack.local-ref"))?.event;
    ...
    async #reattachInterruptedExecution(item, attempt) {
        ...
        const request = this.#implementationRequest(item, attempt.attemptId, lease.worktreePath, storedContext, attempt.contextHash, attempt.deadline);
    ...
    async #validateVerifiedCheckpointEvidence(item, checkpoint) {
        ...
        const requiredGateIds = new Set(restackGateIds ?? item.acceptance.flatMap((predicate) => predicate.type === "gate-passed" ? [predicate.gateId] : []));
        ...
        for (const receiptEvent of receiptEvents) {
            ...
            const waiverValid = waiver !== undefined && gate?.type !== "review"
                ...
                && waiver.alternativeGateIds.every((alternativeGateId) => receiptEvents.some((event) => event.gateId === alternativeGateId && event.status === "PASSED"));
    ...
    async #completeVerifiedRestackItem(item) {
        ...
        if (localCommit === descendant.oldCommit) {
            await this.#withRepositoryLock(async () => {
                ...
                await installRestackCandidate(this.#charter.repository.root, item.branchName, retainedWorktreePath, temporaryWorktreePath, descendant.oldCommit, checkpoint.headCommit, checkpoint.treeIdentity, () => {
                    if (this.#stopRequested || this.#pauseRequested) {
        ...
        await pushAmendmentBranch(retainedWorktreePath, descendant.remote, item.branchName, descendant.remoteCommit, checkpoint.headCommit, () => {
            if (this.#stopRequested || this.#pauseRequested) {
    ...
    async #runRestackItem(item) {
        ...
        if (projected.state === "PREPARING") {
            ...
            const candidate = await this.#withRepositoryLock(async () => {
                ...
                return await prepareRestackCandidate(this.#charter.repository.root, this.#charter.runId, item.id, descendant.oldCommit, freshParentCommit, descendant.worktreePath);
            });
        ...
        if (projected.state === "VERIFYING") {
            ...
            try {
                ...
                const recovered = await this.#withRepositoryLock(async () => await prepareRestackCandidate(this.#charter.repository.root, this.#charter.runId, item.id, descendant.oldCommit, freshParentCommit, descendant.worktreePath));
            ...
            const receiptIds = [...new Set(this.#records.flatMap(({ event }) => event.type === "RECEIPT_RECORDED" && event.itemId === item.id && event.attemptId === verificationId
                    ? [event.receiptId]
                    : []))];
            await this.#record({
