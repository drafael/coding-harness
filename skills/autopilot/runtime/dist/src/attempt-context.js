...
export function buildAttemptContext(input) {
    const { charter, item, attemptId, leaseEpoch, observation, records, projection, predicateEvidence, reviewFindings, sensitiveValues = [], } = input;
    ...
    const dependencyCommits = item.dependsOn.flatMap((dependencyId) => {
        const commit = records.flatMap(({ event }) => event.type === "EFFECT_CONFIRMED" && event.itemId === dependencyId && event.effect === "git.commit"
            ? [event.observedState]
            : []).at(-1);
    ...
    const evidence = predicateEvidence
        ...
        receiptIds: [entry.evaluationReceiptId, ...entry.evidenceReceiptIds].filter((id) => id !== null),
        observed: entry.observed,
        expected: entry.expected,
    }));
    const priorFailures = records.flatMap(({ event }) => event.type === "ITEM_BLOCKED" && event.itemId === item.id
        ? [{
...
export function attemptContextHash(context) {
    return sha256(canonicalJson(context));
}
export function renderAttemptContext(context) {
    return [
...
export function renderReviewContext(context, focus) {
    return [
