...
async function listRunEntries(stateRoot) {
    try {
        const names = [];
        const corrupt = [];
...
export async function locateStoredRun(stateRoot, runId) {
    ...
    const charter = parseSealedCharter(JSON.parse(await readFile(join(directory, "charter.json"), "utf8")));
...
export async function loadStoredRun(stateRoot, runId) {
...
function confirmedEffect(run, itemId, effect) {
    return run.records.flatMap(({ event }) => event.type === "EFFECT_CONFIRMED" && event.itemId === itemId && event.effect === effect
        ? [event.observedState]
        : []).at(-1);
}
function missingPredecessorIsWrapUpRecovery(run, predecessorId) {
    return run.records.some(({ event }) => event.type === "WRAP_UP_STARTED"
        && event.chainRunIds.includes(run.charter.runId)
        && event.chainRunIds.includes(predecessorId));
}
function amendmentIssue(successor, predecessor) {
...
export async function loadAvailableRuns(stateRoot) {
    const entries = await listRunEntries(stateRoot);
    const loaded = [];
    const corrupt = [...entries.corrupt];
    ...
    const invalidSuccessors = new Set();
    let changed = true;
    while (changed) {
        changed = false;
        acyclic.forEach((run) => {
    ...
    const supersededRunIds = new Set(runs.flatMap(({ charter }) => charter.amends !== undefined && validIds.has(charter.amends.runId) ? [charter.amends.runId] : []));
    corrupt.sort((left, right) => left.name.localeCompare(right.name));
...
async function coordinatorState(directory) {
...
function exclusionReason(operation, candidate, statusHasNonterminal) {
    if (operation === "status") {
...
export async function discoverLifecycleRuns(stateRoot, repositoryRoot, operation) {
    const canonicalRepository = await canonicalRepositoryRoot(repositoryRoot);
    const available = await loadAvailableRuns(stateRoot);
    const leaves = available.runs.filter(({ charter }) => charter.repository.root === canonicalRepository && !available.supersededRunIds.has(charter.runId));
    const ids = shortIds(leaves.map(({ charter }) => charter.runId));
    const summaries = await Promise.all(leaves.map(async (run) => ({
