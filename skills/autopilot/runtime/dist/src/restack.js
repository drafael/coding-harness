...
const GRANT_LIST_FIELDS = ["commands", "repositories", "remotes", "environmentNames"];
function pathWithin(candidate, root) {
    const path = relative(resolve(root), resolve(candidate));
    return path === "" || (!path.startsWith("..") && !isAbsolute(path));
}
function grantCoveredBy(candidate, authority) {
    ...
    return pathsCovered && branchesCovered && GRANT_LIST_FIELDS.every((field) => authority[field] === undefined
        || (candidate[field] !== undefined && candidate[field].every((value) => authority[field]?.includes(value))));
}
export function grantsWithinRestackAuthority(grants, sourceGrants, amendmentGrants) {
    return grants.every((grant) => sourceGrants.some((authority) => grantCoveredBy(grant, authority))
        && amendmentGrants.some((authority) => grantCoveredBy(grant, authority)));
}
function confirmedState(records, itemId, effect) {
    return records.flatMap(({ event }) => event.type === "EFFECT_CONFIRMED" && event.itemId === itemId && event.effect === effect
        ? [event.observedState]
        : []).at(-1);
}
async function loadSuccessfulRun(stateRoot, runId) {
...
export async function validateRestackSuccessor(stateRoot, charter) {
