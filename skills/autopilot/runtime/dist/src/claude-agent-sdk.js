...
const CLAUDE_AGENT_SDK_SCRIPT_SUFFIXES = [".js", ".mjs", ".tsx", ".ts", ".jsx"];
...
export function isClaudeAgentSdkScriptCli(cliPath) {
    return CLAUDE_AGENT_SDK_SCRIPT_SUFFIXES.some((suffix) => cliPath.endsWith(suffix));
}
export function isSupportedClaudeAgentSdkVersion(value) {
...
export async function inspectClaudeAgentSdkInstallation(rootValue = process.env[CLAUDE_AGENT_SDK_ROOT_ENVIRONMENT], cliValue = process.env[CLAUDE_AGENT_SDK_CLI_ENVIRONMENT]) {
    if (rootValue === undefined || rootValue.length === 0 || cliValue === undefined || cliValue.length === 0) {
        throw new AutopilotError("ADAPTER_UNSUPPORTED", `${CLAUDE_AGENT_SDK_ROOT_ENVIRONMENT} and ${CLAUDE_AGENT_SDK_CLI_ENVIRONMENT} must name an operator-provided Agent SDK and Claude Code executable`);
    }
    let root;
    let cliPath;
    try {
    ...
    if (!isSupportedClaudeAgentSdkVersion(packageValue.version)) {
        throw new AutopilotError("ADAPTER_UNSUPPORTED", `Claude Agent SDK ${packageValue.version} is older than ${MINIMUM_CLAUDE_AGENT_SDK_VERSION}`);
    }
    try {
