...
function installationAt(extensionPath) {
    if (!existsSync(extensionPath)) {
        return undefined;
    }
    try {
        const packageValue = JSON.parse(readFileSync(join(dirname(extensionPath), "package.json"), "utf8"));
...
export function findPiSubagentsInstallation(cwd = process.cwd()) {
