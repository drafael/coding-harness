...
interface HttpClient {
  ...
  readonly eventResponse: () => Promise<Response>;
}
...
function isTerminalAssistant(info: Record<string, unknown>, messageId: string): boolean {
...
function textOutput(value: unknown): string {
  ...
  return parts.flatMap((part): readonly string[] =>
    isRecord(part) && part.type === "text" && typeof part.text === "string" ? [part.text] : []
  ).join("");
}

async function responseBody(response: Response, maximumBytes: number, label: string): Promise<string> {
...
function createHttpClient(options: {
  ...
  return {
    ...
    async eventResponse(): Promise<Response> {
...
async function consumeEvents(options: {
  ...
  readonly onEvent: (value: unknown) => Promise<void>;
}): Promise<never> {
...
async function validatePath(value: unknown, worktreePath: string): Promise<void> {
...
export class OpenCodeServerAdapter implements HarnessPort {
  readonly #options: OpenCodeServerAdapterOptions;
  ...
  async launch(request: ExecutionRequest): Promise<ExecutionHandle> {
    ...
    const startupUrl = deferred<string>();
    ...
    let executionUnknown: AutopilotError | undefined;
    let seenBusy = false;
    ...
    const assistantInfos = new Map<string, Record<string, unknown>>();
    ...
    const maximumResponseBytes = Math.max(
      ...
      Buffer.byteLength(prompt) + request.maximumLineBytes,
    );
    ...
    const assertAdmissionContinuity = (): void => {
      if (executionUnknown !== undefined) {
        throw executionUnknown;
      }
    };
    const resolveTerminal = (observation: ExecutionObservation): void => {
      if (terminalAccepted) {
        return;
      }
      terminalAccepted = true;
    ...
    const reconcile = async (): Promise<void> => {
      ...
      try {
        ...
        const assistant = requiredRecord(
          await client.json(`/session/${sessionId}/message/${assistantId}`),
        ...
        const status: ExecutionObservation["status"] = assistantInfo.error === undefined
    ...
    const rejectInteractiveRequest = async (type: string, properties: Record<string, unknown>): Promise<void> => {
    ...
    const onEvent = async (value: unknown): Promise<void> => {
    ...
    try {
      ...
      child.stderr.on("data", (chunk: Buffer) => appendStderr(stderrRedactor.write(chunk)));
      ...
      throw new AutopilotError(
        "EXECUTION_STATE_UNKNOWN",
        "OpenCode server admission did not return an exact session and message identity",
        { cause: errorMessage(error) },
      );
    }
  }
  ...
  async cancel(handle: ExecutionHandle): Promise<CancelResult> {
    ...
    const abort = pending.abort().then(() => ({ kind: "response" as const }));
    const terminal = pending.terminal.then((observation) => ({ kind: "terminal" as const, observation }));
