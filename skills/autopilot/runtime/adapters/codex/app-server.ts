...
async function threadIdentity(value: unknown, worktreePath: string): Promise<string> {
...
function turnIdentity(value: unknown): string {
...
function turnFromNotification(value: unknown): {
  readonly threadId: string;
  readonly turnId: string;
  readonly status: string;
  readonly error?: string;
  readonly agentOutput: readonly string[];
} {
  ...
  const agentOutput = items.flatMap((item): readonly string[] =>
    isRecord(item) && item.type === "agentMessage" && typeof item.text === "string" ? [item.text] : []
  );
...
class AppServerConnection {
  ...
  readonly #pendingRequests = new Map<number, RpcPendingRequest>();
  ...
  readonly #onProcessClose = (code: number | null, signal: NodeJS.Signals | null): void => {
    if (!this.#closed) {
      this.#fail(new AutopilotError(
        "EXECUTION_STATE_UNKNOWN",
        "Codex app-server exited before the exact turn reached terminal state",
        { code, signal },
      ));
    }
  };
...
export class CodexAppServerAdapter implements HarnessPort {
  readonly #options: CodexAppServerAdapterOptions;
  ...
  async launch(request: ExecutionRequest): Promise<ExecutionHandle> {
    ...
    let provisionalTurnId: string | undefined;
    let earlyTerminal: unknown;
    let output = "";
    let outputTruncated = false;
    let terminalAccepted = false;
    let failure: AutopilotError | undefined;
    ...
    const observeTurnId = (value: unknown): string | undefined => {
    ...
    const acceptTerminal = (params: unknown): void => {
      if (terminalAccepted) {
        return;
      }
      ...
      if (!(["completed", "interrupted", "failed"] as const).includes(
        completed.status as "completed" | "interrupted" | "failed",
      )) {
      ...
      const status: ExecutionObservation["status"] = completed.status === "completed"
        ? "completed" : completed.status === "interrupted" ? "cancelled" : "failed";
      const stderrOutput = boundUtf8(
        completed.error === undefined
          ? stderr.value
          : `${stderr.value}${redactSecrets(completed.error, credentials)}`,
    ...
    try {
      ...
      throw new AutopilotError(
        "EXECUTION_STATE_UNKNOWN",
        "Codex app-server admission did not return an exact thread and turn identity",
        { cause: errorMessage(error) },
      );
    }
  }
