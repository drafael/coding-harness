...
interface DelegationUpdate extends EventIdentity {
...
interface DelegationResponse extends EventIdentity {
...
interface PiExtensionApi {
  ...
  registerCommand(name: string, options: {
    ...
    readonly handler: (arguments_: string, context: { readonly cwd: string }) => Promise<void>;
  }): void;
  sendMessage(message: {
...
function matches(value: unknown, requestId: string, ownerRunId: string, nodeId: string): value is Record<string, unknown> {
  return isRecord(value) && value.requestId === requestId && value.ownerRunId === ownerRunId && value.nodeId === nodeId;
}

function decodePayload(value: string): BridgePayload {
  const parsed: unknown = JSON.parse(Buffer.from(value.trim(), "base64url").toString("utf8"));
...
function activity(update: DelegationUpdate): string {
  return [
    update.currentTool,
...
async function delegate(pi: PiExtensionApi, context: { readonly cwd: string }, payload: BridgePayload): Promise<DelegationResponse> {
  const requestId = randomUUID();
  const ownerRunId = payload.runId;
  const nodeId = payload.itemId;
  let lastActivity = "";
  let lastActivityAt = 0;
  return await new Promise<DelegationResponse>((resolvePromise, rejectPromise) => {
    const cleanups: Array<() => void> = [];
    const cleanup = (): void => cleanups.splice(0).forEach((dispose) => dispose());
    ...
    cleanups.push(pi.events.on(RESPONSE_EVENT, (value) => {
      ...
      const result = isRecord(value.result) && (value.result.kind === "text" || value.result.kind === "structured")
        ...
        : undefined;
      const event: DelegationResponse = {
        requestId,
