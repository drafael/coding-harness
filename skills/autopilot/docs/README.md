...
The runtime has 133 passing Node tests covering deterministic attempt context, predicate evidence maps, exact-tree review fixtures, local Git lifecycle behavior, crash reconciliation, intentional pause, exact-subject provider waiting, hooks, queues, stacks, sealed review-feedback amendments, GitHub and GitLab provider contracts, sibling worktrees, and wrap-up. GitHub PR creation, marker reconciliation, exact review-thread resolution, and exact-head merge passed on an authorized private disposable target with `gh` 2.98.0. Live harness review, the complete review-feedback amendment workflow, and GitLab mutation remain unverified.
