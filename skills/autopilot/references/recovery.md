...
`run.lock` is an atomic directory containing owner host, PID, start time, and token. A second live coordinator is denied. A dead same-host PID can be replaced through an atomic stale-lock rename. Node 24 CI exercises same-host locking and stale-state behavior on Windows; cross-host replacement remains unverified.
